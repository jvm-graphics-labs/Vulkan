...
//        device.waitForFences(fence, true, DEFAULT_FENCE_TIMEOUT)
//
...
//        device.waitForFences(waitFences[currentBuffer], true, UINT64_MAX)
...
//                +0f, -1f, +0f, 0f, 0f, 1f)
//
//        val vertexBufferSize = VkDeviceSize(vertexBuffer.rem)
//
//        // Setup indices
//        val indexBuffer = it.ints(0, 1, 2)
//        indices.count = indexBuffer.cap
//        val indexBufferSize = VkDeviceSize(indexBuffer.rem)
//
...
//            // Map and copy
//            device.mappedMemory(stagingBuffers.vertices.memory, VkDeviceSize(0), memAlloc.allocationSize) { data ->
...
//            device.mappedMemory(stagingBuffers.indices.memory, VkDeviceSize(0), indexBufferSize) { data ->
...
//            }
//            vertices.memory = device allocateMemory memAlloc
//            device.mappedMemory(vertices.memory, VkDeviceSize(0), memAlloc.allocationSize) { data ->
...
//            memAlloc.memoryTypeIndex = getMemoryTypeIndex(memReqs.memoryTypeBits, memoryPropertiesFlags)
//            indices.memory = device allocateMemory memAlloc
//            device.mappedMemory(indices.memory, VkDeviceSize(0), indexBufferSize) { data ->
//                memCopy(indexBuffer.adr, data, indexBufferSize)
//            }
//            device.bindBufferMemory(indices.buffer, indices.memory)
//        }
...
//            stak {
//                val attachments = it.VkImageView_BufferOf(
...
//                samples = VkSampleCount._1_BIT                 // We don't use multi sampling in this example
...
//        colorReference[0].attachment = 0                                      // Attachment 0 is color
//        colorReference[0].layout = VkImageLayout.COLOR_ATTACHMENT_OPTIMAL     // Attachment layout used as color during the subpass
//
...
//        shaderStages.forEach { device destroyShaderModule it.module } // TODO improve
//    }
//
...
//        device.mappedMemory(uniformBufferVS.memory, VkDeviceSize(0), VkDeviceSize(uboVS.size), 0) { data ->
//            uboVS to data
